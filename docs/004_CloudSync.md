...
Version 2.2.2 uses Supabase while retaining LocalStorage as the immediate, offline-capable store. Account settings include password-manager integration, authenticated password changes, and email password recovery. Signed-out users are locked to the account screen so device-local data is not exposed. PWA updates use only the active version cache.
