...
export default function PrivacyPolicy({ onBack }: Props) {
  ...
  if (language === "en") return <article className="privacy-policy">
    ...
    <div className="privacy-policy-body">
      ...
      <section><h3>4. Usage analytics</h3><p>For service operation, Shoot Log records the app version, display language, browser/PWA launch mode, launch count, and last-used time for signed-in accounts. Shooting records, email addresses, device identifiers, and precise location are not included in the administrator analytics.</p></section>
      <section><h3>5. External services</h3><ul><li><a href="https://supabase.com/privacy" target="_blank" rel="noreferrer">Supabase</a>: authentication, cloud database, sync, and anonymous usage aggregation.</li><li><a href="https://docs.github.com/en/site-policy/privacy-policies/github-general-privacy-statement" target="_blank" rel="noreferrer">GitHub Pages</a>: app hosting and delivery.</li><li><a href="https://policies.google.com/privacy?hl=en" target="_blank" rel="noreferrer">Google</a>: authentication email delivery and support email receipt.</li></ul><p>Information may be processed or stored outside Japan. The Operator uses these providers only as necessary after reviewing their contracts, privacy information, and security practices.</p></section>
  ...
  return <article className="privacy-policy">
    ...
    <div className="privacy-policy-body">
      ...
      <section><h3>4. 利用状況の集計</h3><p>サービス運営のため、ログイン中のアカウントについて、アプリのバージョン、表示言語、ブラウザー／PWAの起動形式、起動回数および最終利用日時を記録します。管理者向け集計には、射撃記録、メールアドレス、端末識別情報および正確な位置情報を含めません。</p></section>

      <section><h3>5. 外部サービス</h3><p>本サービスは、提供に必要な範囲で次の外部サービスを利用します。</p><ul>
        <li><a href="https://supabase.com/privacy" target="_blank" rel="noreferrer">Supabase</a>：ユーザー認証、クラウドデータベース、同期および匿名利用集計</li>
