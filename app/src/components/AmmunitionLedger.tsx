...
function LedgerSettings({ data, ammunitionNames, onChange }: { data: AmmunitionLedgerData; ammunitionNames: string[]; onChange: Props["onChange"] }) {
  ...
  return <div className="ammo-settings">
    ...
    <section><header><h3>使用銃</h3><span>所持許可管理と共通</span></header><p className="firearm-management-note">銃の追加・修正・更新期限は、履歴画面上部の「所持許可管理」で行います。</p><ul>{data.firearms.map((item) => <li key={item.id}><span><strong>{item.name}</strong><small>{item.identifier}</small></span></li>)}</ul></section>
