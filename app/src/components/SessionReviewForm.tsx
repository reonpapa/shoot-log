...
interface Props { review: SessionReview; onSave: (review: SessionReview) => void; }
...
export function SessionReviewForm({ review, onSave }: Props) {
  const [form, setForm] = useState(review);
  const [saved, setSaved] = useState(false);
  function update(key: keyof SessionReview, value: string) { setForm((current) => ({ ...current, [key]: value })); setSaved(false); }
  function save() { onSave({ findings: form.findings.trim(), problems: form.problems.trim(), nextChallenge: form.nextChallenge.trim() }); setSaved(true); }
