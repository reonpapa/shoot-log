...
export function StandRadialChart({ stats, directionScaleMax }: Props) {
  ...
  return <article className="stand-radial"><svg viewBox="0 0 140 140" role="img" aria-label={text(`射台${stats.standNo}、命中率${Math.round(hitRate * 100)}パーセント、初矢命中率${Math.round(firstRate * 100)}パーセント、左失中${left}、中央失中${center}、右失中${right}`, `Stand ${stats.standNo}, hit rate ${Math.round(hitRate * 100)} percent, first-shot hit rate ${Math.round(firstRate * 100)} percent, misses left ${left}, center ${center}, right ${right}`)}>
    ...
    <text className="stand-label" x="70" y="80">{text("射台", "STAND")}</text><text className="stand-number" x="70" y="95">{stats.standNo}</text>
