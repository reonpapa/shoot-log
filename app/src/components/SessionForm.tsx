...
export function SessionForm({ onStart, onCancel, cancelLabel = "キャンセル", initialValue, title = "新しい射撃", kicker = "NEW SESSION", submitLabel = "セッション開始", rangeNames = [], ammunitionNames = [], firearms = [], practiceRecommendation = null }: Props) {
  ...
  return <section className="session-form">
    ...
    <form onSubmit={submit}>
      ...
      {showRecommendation && practiceRecommendation && <section className="wide next-practice-navigator" aria-label={text("次回練習ナビ", "Next practice guide")}>
        <header><div><p className="eyebrow">NEXT PRACTICE</p><strong>{text("次回練習ナビ", "Next practice guide")}</strong></div><span>{text("おすすめ", "Suggested")}</span></header>
        <h3>{practiceRecommendation.theme}</h3>
        <p>{practiceRecommendation.reason}</p>
        <div><button type="button" className={recommendationAccepted ? "accepted" : ""} onClick={() => { update("practiceTheme", practiceRecommendation.theme); setRecommendationAccepted(true); }}>{recommendationAccepted ? text("採用しました", "Selected") : text("このテーマを採用", "Use this focus")}</button><button type="button" onClick={() => { update("practiceTheme", practiceRecommendation.theme); practiceThemeRef.current?.focus(); }}>{text("編集して採用", "Edit and use")}</button><button type="button" onClick={() => { if (form.practiceTheme === practiceRecommendation.theme) update("practiceTheme", ""); setShowRecommendation(false); }}>{text("今回は使わない", "Not this time")}</button></div>
      </section>}
